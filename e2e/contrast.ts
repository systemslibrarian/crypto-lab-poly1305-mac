 ...
 *    files the node under `incomplete`. The action buttons are
 *    `linear-gradient` fills and several panels are translucent tints, so their
 *    text composites onto whatever is behind rather than a flat colour.
 ...
 *    `--ink-soft` a further 85%, which is how it reached 3.47:1.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. The accumulator
 *    table is `min-width: 920px` inside a `.table-shell` scroller, so at 380px
 *    most of its columns are clipped away. Content scrolled past the client box is not dimmed or partly
