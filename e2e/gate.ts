 ...
 *
 *  1. NOTHING IS INJECTED INTO THE PAGE BEFORE A SCAN. The three info tabs are
 *     mutually exclusive by design, so they are visited one at a time rather
 *     than force-revealed together.
 *
 ...
 *     nothing, and every verdict here — the three verification outcomes and the
 *     nonce-reuse forgery — renders only after its button is pressed.
 *
...
export async function boot(page: Page, theme: 'dark' | 'light'): Promise<void> {
  ...
  await expect(page.locator('#generate-key-button')).toBeVisible();
 ...
 * plausible offender: it prints the 32-byte key, r, and 16-byte tags as
 * unbroken hex, and the math stepper lays out wide equations.
 */
 ...
 *
 * All three verification outcomes are driven, not just the passing one: a
 * tampered message and a tampered tag are separately styled failure verdicts,
 * and a gate that only scans the happy path never sees either. The nonce-reuse
 * forgery is the lab's headline claim and renders nothing until it runs.
 */
export async function driveAllStates(page: Page, theme: string): Promise<void> {
  await scan(page, `${theme} / first paint`);

  await page.locator('#generate-key-button').click();
  await expect(page.locator('#key-display')).not.toBeEmpty();
  await expect(page.locator('#r-hex-display')).not.toBeEmpty();
  await scan(page, `${theme} / key generated`);

  await page.locator('#message-input').fill('authenticate this message');
  await page.locator('#compute-mac-button').click();
  await expect(page.locator('#tag-display')).not.toBeEmpty();
  await scan(page, `${theme} / tag computed`);

  // The three verification branches.
  await page.locator('#verify-original-button').click();
  await expect(page.locator('#verify-original-status')).not.toBeEmpty();
  await scan(page, `${theme} / original verifies`);

  await page.locator('#verify-message-button').click();
  await expect(page.locator('#verify-message-status')).not.toBeEmpty();
  await scan(page, `${theme} / tampered message rejected`);

  await page.locator('#verify-tag-button').click();
  await expect(page.locator('#verify-tag-status')).not.toBeEmpty();
  await scan(page, `${theme} / tampered tag rejected`);

  // The math stepper.
  await page.locator('#show-math-button').click();
  await expect(page.locator('#math-steps-body')).not.toBeEmpty();
  await scan(page, `${theme} / math steps`);

  // Nonce reuse, then the forgery it enables — the lab's headline claim.
  await page.locator('#compute-reuse-button').click();
  await expect(page.locator('#reuse-tag-one')).not.toBeEmpty();
  await expect(page.locator('#reuse-tag-two')).not.toBeEmpty();
  await scan(page, `${theme} / key reused across two messages`);

  await page.locator('#forge-message-input').fill('forged content');
  await page.locator('#forge-button').click();
  await expect(page.locator('#forge-status')).not.toBeEmpty();
  await scan(page, `${theme} / forged tag`);

  // The three info tabs, one at a time — they are mutually exclusive.
  for (const tab of ['tab-what', 'tab-math', 'tab-constant-time']) {
    await page.locator(`#${tab}`).click();
    await expect(page.locator(`#${tab}`)).toHaveAttribute('aria-selected', 'true');
    await scan(page, `${theme} / info tab: ${tab}`);
  }
}
